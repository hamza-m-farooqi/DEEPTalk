...
'''
# sys.path.append(f'/home/{user_name}/talkinghead_dataset/workspace/audio2mesh/emoca-ydc/external/spectre/external/Visual_Speech_Recognition_for_Multiple_Languages')
...
def calc_vae_loss_with_facemask(pred,target,mu, logvar, recon_weight=1000000, kl_weight=0.001):                            
    """ function that computes the various components of the VAE loss """
    f = '/workspace/audio2mesh/EMOTE/models/flame_models/masks/FLAME_masks.pkl'
    with open(f, 'rb') as file:
        masks = pickle.load(file, encoding='latin1')
    BS, T, _ = pred.shape

    pred = pred.view(BS,T, -1,3) # (BS,T,5023,3)
    target = target.view(BS,T, -1,3) # (BS,T,5023,3)
    # mask along the vertex indexes
    pred = pred[:,:,masks['face'],:]
    target = target[:,:,masks['face'],:]
    pred = pred.view(BS,T,-1)
    target = target.view(BS,T,-1)
...
def calculate_flame_jaw_loss(pred, target):
    jaw_loss = nn.MSELoss()(pred[:,:,50:53], target[:,:,50:53])
    return jaw_loss
...
if __name__ == '__main__' :
    with open('/workspace/audio2mesh/EMOTE/configs/EMOTE/EMOTE_inferno.json') as f:
        EMOTE_config = json.load(f)

    video_emotion_config = EMOTE_config['loss']['emotion_video_loss']
    # create_video_emotion